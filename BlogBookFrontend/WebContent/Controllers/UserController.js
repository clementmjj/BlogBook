...
if($location.path()=="/showUser")
	...
	$scope.register = function() {
		...
		$http.post('http://localhost:' + location.port + '/BlogBookMiddleware/registerUser', $scope.registerUser).then(function(response) {
	...
	$scope.loginCheck = function() {
		$http.post('http://localhost:' + location.port + '/BlogBookMiddleware/checkLogin', $scope.user).then(function() {
			console.log('login successfull');
			$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getUser/' + $scope.user.username).then(function(response) {
				var user=response.data;
				$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getProfilePic/' + user.username).then(function(response) {
					if(response.data){
					...
					window.location.href = "http://localhost:" + location.port + "/BlogBookFrontend/index2.html#/blog";
	...
	{	
		$http.post('http://localhost:' + location.port + '/BlogBookMiddleware/updateUser',$scope.user).then(function(response){
