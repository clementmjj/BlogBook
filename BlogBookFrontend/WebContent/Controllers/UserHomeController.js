myApp.controller("UserHomeController", function($scope, $http, $location, $rootScope) {
	...
	$scope.generateNewsFeed = function() {
		...
		$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getLimitedBlogList/' + $rootScope.currentUser.username + '/' + $scope.blogStartRowNum + '/' + $scope.blogEndRowNum).then(
				function(response) {
					...
					$http.get(
							'http://localhost:' + location.port + '/BlogBookMiddleware/getLimitedForumList/' + $rootScope.currentUser.username + '/' + $scope.forumStartRowNum + '/'
							...
							function(response) {
								...
								$http.get(
										'http://localhost:' + location.port + '/BlogBookMiddleware/getLimitedJobList/' + $rootScope.currentUser.username + '/' + $scope.jobStartRowNum + '/'
										...
										function(response) {
											...
											// of each item
											$scope.newsFeedItems.forEach(function(item) {
												$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getUser/' + item.username).then(
														function(response) {
															...
															if (item.type == 'blog' || item.type == 'forum') {
																$http.get(
																		'http://localhost:' + location.port + '/BlogBookMiddleware/'
																	...
																	response.data.forEach(function(comment) {
																		$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getUser/' + comment.username).then(function(response) {
