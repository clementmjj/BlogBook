myApp.controller("ForumCommentController", function($scope, $http, $location, $rootScope, $cookieStore, $compile) {
	...
	$scope.addForumComment = function() {
		...
		$http.post('http://localhost:' + location.port + '/BlogBookMiddleware/addForumComment', $scope.forumComment).then(function(comment) {
	...
	$scope.getForumCommentList = function(forumId) {
		$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getForumCommentList/' + forumId).then(function(response) {
			...
			$scope.forumCommentList.forEach(function(forumComment) {
				$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getUser/' + forumComment.username).then(function(response) {
					forumComment.firstName = response.data.firstName;
					forumComment.lastName = response.data.lastName;
				});
			});
			$scope.forumCommentList.forEach(function(forumComment) {
				$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getProfilePic/' + forumComment.username).then(function(response) {
					if (response.data) {
						forumComment.profilePicUrl = 'http://localhost:' + location.port + '/BlogBookMiddleware/showProfilePic/' + forumComment.username;
					} else {
						forumComment.profilePicUrl = 'http://localhost:' + location.port + '/BlogBookFrontend/Resources/Images/empty-profile-pic.png';
					}
				});
			});
	...
	$scope.deleteForumComment = function(forumCommentId) {
		$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/deleteForumComment/' + forumCommentId).then(function(response) {
	...
	$scope.editForumComment = function(forumCommentId) {
		...
		if (btn_editSaveComment.getAttribute("title") == "Save") {
			...
			$http.post('http://localhost:' + location.port + '/BlogBookMiddleware/editForumComment', $scope.forumCommentEdit).then(function(response) {
