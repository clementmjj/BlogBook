myApp.controller("SearchController", function($scope, $http, $location, $rootScope, $cookieStore) {
	...
	$scope.search = function() {
		...
		if ((searchBar.value.trim()).length > 0) {
			...
			// blog search
			$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/blogSearch/' + $scope.searchText).then(
					function(response) {
						response.data.forEach(function(item) {
							$rootScope.blogResults.push(item);
						});
						// forum search
						$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/forumSearch/' + $scope.searchText).then(
								function(response) {
									response.data.forEach(function(item) {
										$rootScope.forumResults.push(item);
									});
									// job search
									$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/jobSearch/' + $scope.searchText).then(
											function(response) {
												response.data.forEach(function(item) {
													$rootScope.jobResults.push(item);
												});
												// user search
												$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/userSearch/' + $scope.searchText).then(
														function(response) {
															...
															$rootScope.userResults.forEach(function(user) {
																$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getProfilePic/' + user.username).then(
																		function(response) {
																			if (response.data) {
																				user.profilePicUrl = 'http://localhost:' + location.port + '/BlogBookMiddleware/showProfilePic/' + user.username;
																			} else {
																				user.profilePicUrl = 'http://localhost:' + location.port + '/BlogBookFrontend/Resources/Images/empty-profile-pic.png';
																			}
																			...
																			// status
																			$http.get(
																					'http://localhost:' + location.port + '/BlogBookMiddleware/checkIfFriends/' + $rootScope.currentUser.username + '/'
