...
myApp.controller("ProfilePicController", function($scope, $http, $location, $rootScope, $cookieStore) {
	...
	$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/getProfilePic/' + $rootScope.currentUser.username).then(function(response) {
		...
		if ($scope.profilePic) {
			...
			$rootScope.currentUser.profilePicUrl = 'http://localhost:' + location.port + '/BlogBookMiddleware/showProfilePic/' + $rootScope.currentUser.username;
			$cookieStore.put('userDetails', $rootScope.currentUser);
		} else {
			console.log("pp not found");
			$rootScope.currentUser.profilePicUrl = 'http://localhost:' + location.port + '/BlogBookFrontend/Resources/Images/empty-profile-pic.png';
		}
	});
	...
	$scope.deleteProfilePic = function() {
		$http.get('http://localhost:' + location.port + '/BlogBookMiddleware/deleteProfilePic/' + $rootScope.currentUser.username).then(function(response){
